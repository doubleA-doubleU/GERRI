...
void __ISR(_INPUT_CAPTURE_4_VECTOR, IPL5SOFT) IC4ISR(void) {
    static unsigned int ptsR=0;     // previous time stamp for right sensor
    static int stateR = -1;         // state of right sensor
    unsigned int tsR;               // time stamp for right sensor
    ...
    unusedR = IC4BUF;               // ...
    unusedR = IC4BUF;               // ...
    LATBbits.LATB15=PORTBbits.RB7; // match the sensor signal on a debugging pin
    if (PORTBbits.RB7) {           // ignore rising edges
        IFS0bits.IC4IF = 0;         // clear interrupt flag
        return;                     // exit the ISR
    }    
    tsR = _CP0_GET_COUNT();         // get the current time
    if (ptsR > 0) {                 // won't calculate anything the first time through
        unsigned int r = tsR - ptsR;// calculate elapsed time
        if  (r > CYCLE) {           // indicates a synchronization cycle
            stateR = 0;             // prepare to read y angle (from right to left)
        } else {
            if (stateR >= 0) {
                switch (stateR) {   
                    case 0: {
                        y_ang[1]=r*0.0006;  // read y angle in degrees (0 to 120)
                        break;
                    }
                    case 2: {
                        x_ang[1]=r*0.0006;  // read x angle in degrees (0 to 120)
                        break;
                    }
                }
                stateR++;                       // cycle through states
                if (stateR >=4 ) stateR = -1;   // reset state
            }
        }
    }
    ptsR=tsR;           // set the previous time stamp for the next time through
